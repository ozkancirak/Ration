...
{
    private readonly string _dir = Path.Combine(Path.GetTempPath(), $"ration-sess-{Guid.NewGuid():N}");
    ...
    {
        var source = new CodexSessionLogUsageSource(Path.Combine(Path.GetTempPath(), $"ration-yok-{Guid.NewGuid():N}"));
