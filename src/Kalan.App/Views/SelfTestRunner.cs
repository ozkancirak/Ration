...
internal static class SelfTestRunner
{
    ...
    public static async Task RunAsync(FlyoutWindow flyout, string? screenshotDir)
    {
        ...
        flyout.SelfTestExitRequested = async () =>
        {
            ...
            KalanTrace.Info("selftest", closeOnExit ? "exit-close" : "exit-intercepted");
        ...
        try
        {
            KalanTrace.Info("selftest", "start");

            await OpenMenuAsync(menu, failures);
            KalanTrace.Info("selftest", "pass=mouse");
            ...
            KalanTrace.Info("selftest", "pass=keyboard");
            for (int i = 0; i < buttons.Count; i++)
            {
                bool isExit = i == buttons.Count - 1;
                if (isExit)
        ...
        {
            KalanTrace.Error("selftest", "exit-not-observed");
            Environment.Exit(1);
        }
    }
        ...
        List<string> failures)
    {
        ...
        KalanTrace.Info("selftest", $"input=mouse button={label}");
        ...
        List<string> failures)
    {
        ...
        KalanTrace.Info("selftest", $"input=keyboard button={label} tabs={tabCount}");
    ...
    private static bool SendMouseClick(NativeMethods.RECT rect, List<string> failures, string label)
    {
        ...
        {
            KalanTrace.Info("selftest", $"mouse-after button={label} x={actualCursor.X} y={actualCursor.Y}");
        }
    ...
    private static int CountLog(string marker) =>
        KalanTrace.ReadLastLines(2000).Count(line => line.Contains(marker, StringComparison.Ordinal));
    ...
    private static void ReportFailure(List<string> failures, string message)
    {
        ...
        KalanTrace.Error("selftest", message);
    }
    ...
    private static void PrintResult(List<string> failures, ref bool resultPrinted)
    {
        ...
        if (failures.Count == 0)
        {
            KalanTrace.Info("selftest", "pass");
            Console.WriteLine("SELFTEST PASS");
            return;
        }

        KalanTrace.Error("selftest", $"fail count={failures.Count}");
