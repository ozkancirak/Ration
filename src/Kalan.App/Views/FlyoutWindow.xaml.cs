...
public sealed partial class FlyoutWindow : Window
{
    ...
    private sealed record TabDef(string Id, string Name, string? PathData = null, string? AssetUri = null);

    // Gerçek sağlayıcı işaretleri:
    // - Claude/OpenCode: Simple Icons v16 (CC0)
    // - Codex: Codex'in gerçek tek renk path'i
    ...
    [
        new("claude", "Claude", ClaudePathData),
        new("codex", "Codex", CodexPathData),
        new("antigravity", "Antigravity", null, "ms-appx:///Assets/ProviderAntigravity.png"),
        new("opencode", "OpenCode", "M22 24H2V0h20zM17 4.8H7v14.4h10z"),
    ];
    ...
    private void BuildTabs()
    {
        ...
        foreach (var tab in KnownTabs) EnsureTab(tab.Id, tab.Name);
        UpdateTabs();
    }
    ...
    private void UpdateTabs()
    {
        ...
        foreach (var (id, (button, meter)) in _tabs)
        {
            ...
            {
                SetProviderIconForeground(
                    icon,
                    QuotaVisuals.Fill(
                        selected ? "AccentFillColorDefaultBrush" : "TextFillColorSecondaryBrush"));
            }
    ...
    private void RenderDetail()
    {
        if (!_scheduler.Current.TryGetValue(_selectedId, out var snapshot))
        {
            ...
            DetailIconHost.Content = CreateProviderIcon(GetProviderTab(_selectedId), accent: true);
        ...
        DetailIconHost.Content = CreateProviderIcon(GetProviderTab(snapshot.ProviderId), accent: true);
    ...
    private static TabDef GetProviderTab(string providerId, string? fallbackName = null) =>
        KnownTabs.FirstOrDefault(t => t.Id.Equals(providerId, StringComparison.OrdinalIgnoreCase))
        ?? new TabDef(providerId, fallbackName ?? providerId, FallbackTabPathData);

    private static UIElement CreateProviderIcon(TabDef tab, bool accent = false)
    {
        string brushKey = accent ? "AccentFillColorDefaultBrush" : "TextFillColorSecondaryBrush";
        if (tab.AssetUri is not null)
        {
            ...
            bitmap.Foreground = QuotaVisuals.Fill(brushKey);
            return bitmap;
        }

        var path = (PathIcon)XamlReader.Load(
            $"<PathIcon xmlns=\"http://schemas.microsoft.com/winfx/2006/xaml/presentation\" Data=\"{tab.PathData ?? FallbackTabPathData}\" Width=\"16\" Height=\"16\" VerticalAlignment=\"Center\" />");
        path.Foreground = QuotaVisuals.Fill(brushKey);
        return path;
    }

    private static void SetProviderIconForeground(UIElement icon, Microsoft.UI.Xaml.Media.Brush brush)
    {
        switch (icon)
        {
            case PathIcon path:
                path.Foreground = brush;
                break;
